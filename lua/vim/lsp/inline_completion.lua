-- NOTE:
-- This file intentionally overrides Neovim's built-in
-- `runtime/lua/vim/lsp/inline_completion.lua`.
--
-- Reason:
-- Neovim currently provides `vim.lsp.inline_completion.enable(false)`, which
-- disables the entire inline completion capability (lifecycle, state, requests).
-- For UI integration use cases, this is too coarse: sometimes only the rendering
-- of inline completion virtual text ("ghost text") needs to be toggled, while
-- keeping the capability active.
--
-- A common example is using Copilot both as a completion source (e.g. via cmp/Blink)
-- and as an LSP inline completion provider. In this setup, both systems may render
-- virtual text simultaneously, leading to duplicated suggestions.
--
-- This local override adds a display-only toggle for inline completion virtual
-- text, without disabling the capability itself. It is intended as a temporary
-- workaround until Neovim provides an official API.
--
-- Upstream discussion:
--   https://github.com/neovim/neovim/issues/<ISSUE_NUMBER>
--
-- Once an official solution is available upstream, this file should be removed.
--- @brief
...
function Completor:show(hint)
  self:hide()
  if self.virtual_text_enabled == false then
    return
  end
  ...
  local text = type(insert_text) == "string" and insert_text or tostring(grammar.parse(insert_text.value))
  ...
  if api.nvim_win_get_buf(winid) == self.bufnr then
    local cursor_row, cursor_col = unpack(vim.pos.cursor(api.nvim_win_get_cursor(winid)):to_extmark())
...
function Completor:request(kind)
  for client_id in pairs(self.client_state) do
    ...
    if kind == protocol.InlineCompletionTriggerKind.Invoked and api.nvim_get_mode().mode:match("^v") then
...
function Completor:accept(item)
  ...
  if type(insert_text) == "string" then
    ...
    if range then
      ...
      api.nvim_buf_set_text(self.bufnr, range.start.row, range.start.col, range.end_.row, range.end_.col, lines)
...
function M.display(enable, filter)
  local bufnr = vim._resolve_bufnr(filter and filter.bufnr or 0)
  ...
  completor.virtual_text_enabled = enable
  if not enable then
    completor:hide()
  end
  -- else
